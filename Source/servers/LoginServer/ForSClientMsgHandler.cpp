 ...
 *  客户端请求到ls
 *-----------------------------------------------*/
#define REGISTER_C2L_MESSAGE(msg_idx,cls,handler)\
	{\
	RegisterMessage(msg_idx, sizeof(cls), boost::bind(&ProcClientHandler::handler, ProcClientHandler::Instance(), _1, _2, _3)); \
	}

	REGISTER_C2L_MESSAGE(PRO_C2L_ACCOUNT_LOGIN,	C2LAccountLogin,ReqAccountLogin);
	REGISTER_C2L_MESSAGE(PRO_C2L_ROLE_CREATE,	C2LRoleCreate,	ReqRoleCreate);
	REGISTER_C2L_MESSAGE(PRO_C2L_RAND_NAMES,	C2LRandNames,	ReqRandNames);

#undef REGISTER_C2L_MESSAGE


/*------------------------------------------------
 * DP返回的协议 
 *-----------------------------------------------*/
#define	REGISTER_D2L_MESSAGE(msg_idx,cls,handler) \
	{\
	RegisterMessage(msg_idx, sizeof(cls), boost::bind(&ProcDpHandler::handler, ProcDpHandler::Instance(), _1, _2, _3)); \
	}

	REGISTER_D2L_MESSAGE(PRO_D2L_ACCOUNT_LOGIN,		D2LAccountLogin,	AccountLogin);
	REGISTER_D2L_MESSAGE(PRO_D2L_CHARACTER_LIST,	D2LCharacterList,	CharacterList);
	REGISTER_D2L_MESSAGE(PRO_D2L_ROLE_CREATE_RESULT,D2LRoleCreateResult,RoleCreateResult);
	REGISTER_D2L_MESSAGE(PRO_D2L_NAMES_LIST,		D2LNamesList,		RepNamesList);

#undef REGISTER_D2L_MESSAGE
...
void ForSClientMsgHandler::OnNetMsgEnter(NetSocket& rSocket)
{
	if(m_bFirstMsg)
	{
		ClientCommonHandler::ForWsOnMsgEnter(rSocket,ESERVER_TYPE_LS);
		m_bFirstMsg = false;
	}else
	{
		ClientCommonHandler::ForClientToServerOnMsgEnter(rSocket,ESERVER_TYPE_LS);
	}
}
...
{
	//---------------------------------服务组代码begin-------------------------------
	ClientCommonHandler::ForClientUpdataClientUpConnectInfo(pSession,pMsg,ESERVER_TYPE_LS);
	//---------------------------------服务组代码end---------------------------------
